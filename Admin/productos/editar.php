...
$errores = $_SESSION['errores_producto_editar'] ?? [];
$old = $_SESSION['old_producto_editar'] ?? [];
unset($_SESSION['errores_producto_editar'], $_SESSION['old_producto_editar']);
				...
				<form action="actualizar.php" method="post" enctype="multipart/form-data" class="admin-form">
					...
					<div class="admin-form-grid">
						<div class="admin-field">
							...
							<img src="<?php echo htmlspecialchars($producto['imagen'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" alt="Imagen del producto" class="admin-product-img">
						</div>
						...
						<div class="admin-field">
							...
							<input type="text" id="nombre" name="nombre" maxlength="160" required
								value="<?php echo htmlspecialchars($old['nombre'] ?? $producto['nombre'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
						</div>

						<div class="admin-field admin-field-full">
							<label for="descripcion">Descripción</label>
							<textarea id="descripcion" name="descripcion" rows="5" maxlength="2000" required><?php echo htmlspecialchars($old['descripcion'] ?? ($producto['descripcion'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></textarea>
						</div>

						<div class="admin-field">
							<label for="categoria">Categoria</label>
							<?php $cat = $old['categoria'] ?? ($producto['categoria'] ?? ''); ?>
						...
						<div class="admin-field">
							...
							<input type="number" id="precio" name="precio" min="0" step="1" required
								value="<?php echo htmlspecialchars((string)($old['precio'] ?? $producto['precio'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
						</div>

						<div class="admin-field">
							<label for="stock">Stock</label>
							<input type="number" id="stock" name="stock" min="0" step="1" required
								value="<?php echo htmlspecialchars((string)($old['stock'] ?? $producto['stock'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">
						</div>

						<div class="admin-field">
							<label for="oferta">Oferta</label>
							<?php $oferta = $old['oferta'] ?? (!empty($producto['oferta']) ? 'Si' : 'No'); ?>
						...
						<div class="admin-field">
							...
							<?php $estado = $old['estado'] ?? ($producto['estado'] ?? 'Activo'); ?>
